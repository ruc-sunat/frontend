...
export default function PlanesClient({ initialData }: { initialData: InitialData }) {
  const [planActual, setPlanActual] = useState<number>(initialData.plan_id)
  const [billingCycle, setBillingCycle] = useState<'monthly' | 'annual'>(initialData.billing_cycle)
  const [subscriptionStatus, setSubscriptionStatus] = useState<string>(initialData.subscription_status)
  const [planExpiresAt, setPlanExpiresAt] = useState<string | null>(initialData.plan_expires_at)
  const [selectedCycle, setSelectedCycle] = useState<'monthly' | 'annual'>(initialData.billing_cycle)
  ...
  const effectiveStatus =
    subscriptionStatus !== 'none' && planExpiresAt && new Date(planExpiresAt) < new Date()
      ? 'expired'
      : subscriptionStatus
    ...
    setTimeout(async () => {
      ...
      if (!krInitialized.current) {
        ...
        window.KR.onSubmit(async (paymentData: any) => {
          ...
          const krAnswer = paymentData['kr-answer'] ?? paymentData.rawClientAnswer
  ...
  return (
    <div className="max-w-4xl">
      ...
      <p className="text-sm text-gray-500 mb-6">Elige el plan que se ajuste a tu volumen de consultas.</p>
        ...
        <div className="mb-6 px-4 py-3 rounded-lg text-sm border bg-amber-50 text-amber-800 border-amber-200 flex items-center justify-between">
          <span>
            ...
            <span className="font-semibold">
              {days <= 0 ? 'menos de 1 día' : `${days} día${days > 1 ? 's' : ''}`}
            </span>
            {effectiveStatus === 'cancelled' && '. Renueva para no perder el acceso.'}
          </span>
      ...
      {effectiveStatus !== 'none' && effectiveStatus !== 'expired' && planActual !== null && planActual > 1 && (
        <div className="mb-6 bg-white rounded-xl border border-gray-200 p-5">
          <p className="text-sm font-semibold text-gray-700 mb-3">Tu suscripción</p>
            ...
            {planExpiresAt && (
              <div>
                <span className="text-gray-400">
                  {effectiveStatus === 'cancelled' ? 'Acceso hasta: ' : 'Próxima renovación: '}
                </span>
                <span className="font-medium text-gray-800">{formatDate(planExpiresAt)}</span>
              </div>
            )}
            <div>
              <span
      ...
      <div id="plan-cards" className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {PLANES.map((plan) => {
          const esPlanActualActivo = effectiveStatus === 'active' && planActual === plan.id
          const esPlanCancelado = effectiveStatus === 'cancelled' && planActual === plan.id
          const esMenor = planActual !== null && plan.id < planActual && effectiveStatus === 'active'
          ...
          if (plan.id === 1) {
            ...
            botonLabel = procesando === plan.id ? 'Procesando...' : 'Reactivar plan'
          } else if (esMenor) {
          ...
          return (
            <div
              ...
              className={`bg-white rounded-xl border-2 p-6 flex flex-col ${
                esPlanActualActivo ? 'border-blue-500 shadow-sm' : 'border-gray-200'
              }`}
            >
              {esPlanActualActivo && (
                  ...
                  <div>
                    ...
                    {selectedCycle === 'annual' && (
                      <p className="text-xs text-green-600 mt-0.5">
                        Equivale a S/. {Math.round(getPrecio(plan.id, 'annual') / 12)}/mes
                      </p>
                    )}
                  </div>
                )}
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  ...
                  {selectedCycle === 'annual' ? 'Suscripción anual' : 'Suscripción mensual'}
                </p>
              </div>
              <button
                onClick={() => {
      ...
      {showCancelModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl p-6">
            <h2 className="text-base font-semibold text-gray-900 mb-2">Cancelar suscripción</h2>
